...
def drivetime(start,end):
    r = requests.get('https://maps.googleapis.com/maps/api/directions/json?&origin='+ start +'&destination='+ end +'&departure_time=now&traffic_model=best_guess&key=GOOGLE_KEY')
    output = r.json()
